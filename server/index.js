...
const initializeDatabase = async () => {
  ...
  try {
    ...
    if (brandingCount.rows[0].count === '0') {
        ...
        const initialSettings = { restaurantName: 'Mening Restoranim' };
...
// Categories
app.get('/api/categories', async (req, res) => { try { const r = await pool.query('SELECT * FROM categories ORDER BY sortOrder;'); res.json(r.rows); } catch (e) { res.status(500).json({ error: e.message }); } });
app.post('/api/categories', async (req, res) => { try { const { name, sortOrder, viewType } = req.body; const r = await pool.query('INSERT INTO categories (name, sortOrder, viewType) VALUES ($1, $2, $3) RETURNING *', [name, sortOrder || 0, viewType || 'grid']); res.status(201).json(r.rows[0]); } catch (e) { res.status(500).json({ error: e.message }); } });
app.put('/api/categories/:id', async (req, res) => { try { const { id } = req.params; const { name, sortOrder, viewType } = req.body; const r = await pool.query('UPDATE categories SET name = $1, sortOrder = $2, viewType = $3 WHERE id = $4 RETURNING *', [name, sortOrder, viewType, id]); res.json(r.rows[0]); } catch (e) { res.status(500).json({ error: e.message }); } });
app.delete('/api/categories/:id', async (req, res) => { try { const { id } = req.params; await pool.query('DELETE FROM categories WHERE id = $1', [id]); res.status(204).send(); } catch (e) { res.status(500).json({ error: e.message }); } });
...
// Branches
app.get('/api/branches', async (req, res) => { try { const r = await pool.query('SELECT * FROM branches;'); res.json(r.rows); } catch (e) { res.status(500).json({ error: e.message }); } });
...
app.delete('/api/branches/:id', async (req, res) => { try { const { id } = req.params; await pool.query('DELETE FROM branches WHERE id = $1', [id]); res.status(204).send(); } catch (e) { res.status(500).json({ error: e.message }); } });
